...
function isoToDateInput(iso?: string) {
  if (!iso) return "";
  // API sends "YYYY-MM-DDTHH:MM:SS" -> just take the date part
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCompanyShortForm(data.companyNameShortForm ?? "");
        setBusinessID(data.detailsDto?.businessID ?? "");
        setCountry(data.detailsDto?.country ?? data.country ?? "");
        setWebsite(data.detailsDto?.website ?? "");
        setAddress(data.detailsDto?.address ?? "");
        setCity(data.detailsDto?.city ?? "");
        setPostCode(data.detailsDto?.postCode ?? "");

        // Admin
        setAdminEmail(data.adminEmail ?? data.detailsDto?.email ?? "");
        setAdminFirstName(data.firstName ?? "");
        setAdminLastName(data.lastName ?? "");
        setAdminPhone(data.detailsDto?.mobile ?? data.detailsDto?.phone ?? "");
        // Do NOT prefill password on edit
        setAdminPassword("");

        // Directors
        const mappedDirectors = (data.directors ?? []).map(
          (d: any, i: number) => ({
            id: `${i}-${d.idNumber ?? Math.random()}`,
            fullName: d.fullName ?? "",
            position: d.position ?? "",
            email: d.email ?? "",
            phone: d.phone ?? "",
            ...
            idNumber: d.idNumber ?? "",
            dateOfBirth: isoToDateInput(d.dateOfBirth),
        ...
        // UBOs
        const mappedUbOs = (data.ubOs ?? []).map((u: any, i: number) => ({
          id: `${i}-${u.idNumber ?? Math.random()}`,
          fullName: u.fullName ?? "",
          position: "", // not in payload; leave empty
          email: "", // not in payload; leave empty
          phone: "", // not in payload; leave empty
          ownershipPercentage:
            u.ownershipPercentage != null ? String(u.ownershipPercentage) : "",
          idType: u.idType ?? "National ID",
          idNumber: u.idNumber ?? "",
          dateOfBirth: isoToDateInput(u.dateOfBirth),
        ...
        Swal.fire({
          title: "Error",
          text: err?.message || "Failed to load partner details.",
          icon: "error",
          background: "#1f2937",
          color: "#f9fafb",
        });
            ...
            <div className="flex items-center gap-3">
              {currentStep === totalSteps ? (
                <Button
                  ...
                  className="bg-gradient-to-r from-yellow-600 to-yellow-900 hover:from-green-700 hover:to-emerald-700 dark:bg-green-600 dark:hover:bg-green-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 h-11 px-8 flex items-center gap-2"
                >
